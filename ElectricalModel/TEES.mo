within ;
package TEES
  package building_library_examples
    "these two examples show how to use the battery and pvpanel."
    model Original_AcBattery
...
First implementation.
  ...
  end building_library_examples;

  package PV_battery_nostategraph "this is an original version."
    model model_nostategraph "Pv and battery to test"
      ...
      Buildings.BoundaryConditions.WeatherData.ReaderTMY3 weaDat(
          computeWetBulbTemperature=false, filNam=
            "C:/CoderLife/resilient-building-control/ElectricalModel/weather/USA_TX_Dallas-Love.Field.722583_TMY3.mos")
      ...
      Modelica.Blocks.Interfaces.RealOutput pvpower
        annotation (Placement(transformation(extent={{320,62},{340,82}}),
            iconTransformation(extent={{320,62},{340,82}})));
      Modelica.Blocks.Math.Gain gain(k=-1)
        annotation (Placement(transformation(extent={{32,-108},{46,-94}})));
      Modelica.Blocks.Interfaces.RealOutput soc
        annotation (Placement(transformation(extent={{320,-78},{340,-58}}),
            iconTransformation(extent={{320,-78},{340,-58}})));
    ...
    equation
      ...
      connect(pvSimple.P, add.u1) annotation (Line(points={{31,21},{52,21},{52,
              40},{58,40}},
                        color={0,0,127}));
      ...
      connect(bat_loss_acdc.SOC, less1.u1) annotation (Line(points={{99,-60},{
              118,-60},{118,-40},{154.8,-40}},
                                           color={0,0,127}));
      ...
      connect(pvSimple.P, pvpower) annotation (Line(points={{31,21},{52,21},{52,
              40},{46,40},{46,72},{330,72}},color={0,0,127}));
      connect(gain.y, RL.Pow) annotation (Line(points={{46.7,-101},{56,-101},{
              56,-44},{8,-44}},
                             color={0,0,127}));
      connect(gain.y, add.u2) annotation (Line(points={{46.7,-101},{56,-101},{56,
              28},{58,28}}, color={0,0,127}));
      connect(bat_loss_acdc.SOC, soc) annotation (Line(points={{99,-60},{104,
              -60},{104,-92},{330,-92},{330,-68}},          color={0,0,127}));
    ...
    end model_nostategraph;

    model model_nostategraph_test
      model_nostategraph model_nostategraph1
        annotation (Placement(transformation(extent={{-50,-10},{-2,16}})));
      Modelica.Blocks.Sources.Constant input_for_load(k=600)
        annotation (Placement(transformation(extent={{-96,-8},{-76,12}})));
    equation
      connect(input_for_load.y, model_nostategraph1.Load)
        annotation (Line(points={{-75,2},{-51.1,2.1}}, color={0,0,127}));
      annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
            coordinateSystem(preserveAspectRatio=false)));
    end model_nostategraph_test;
  end PV_battery_nostategraph;

  package PV_battery_stategraph "we try to add stategraph to this model"
    ...
    model model_stategraph "Pv and battery to test"

      Buildings.Electrical.AC.OnePhase.Loads.Inductive RL(
        mode=Buildings.Electrical.Types.Load.VariableZ_P_input,
        P_nominal=-1000,
        V_nominal=120) "Load taht consumes the power generted by the PVs"
        annotation (Placement(transformation(extent={{-4,-56},{22,-32}})));
      Buildings.Electrical.AC.OnePhase.Sources.Grid grid(f=60, V=120)
        "Electrical grid model"
               annotation (Placement(transformation(extent={{-116,-22},{-100,-4}})));
      Buildings.BoundaryConditions.SolarIrradiation.DiffusePerez HDifTil(
        til=0.34906585039887,
        lat=0.65798912800186,
        azi=-0.78539816339745) "Diffuse irradiation on tilted surface"
        annotation (Placement(transformation(extent={{-74,120},{-60,134}})));
      Buildings.BoundaryConditions.SolarIrradiation.DirectTiltedSurface HDirTil(
        til=0.34906585039887,
        lat=0.65798912800186,
        azi=-0.78539816339745) "Direct irradiation on tilted surface"
        annotation (Placement(transformation(extent={{-74,86},{-60,100}})));
      Buildings.BoundaryConditions.WeatherData.ReaderTMY3 weaDat(
          computeWetBulbTemperature=false, filNam=
            "C:/CoderLife/resilient-building-control/ElectricalModel/weather/USA_TX_Dallas-Love.Field.722583_TMY3.mos")
        annotation (Placement(transformation(extent={{-126,98},{-106,118}})));
      Modelica.Blocks.Math.Add G "Total irradiation on tilted surface"
        annotation (Placement(transformation(extent={{-36,104},{-24,116}})));
      Buildings.Electrical.AC.OnePhase.Sources.PVSimple pvSimple(A=10, V_nominal=
            120) "PV array simplified"
        annotation (Placement(transformation(extent={{-2,42},{20,64}})));
      Buildings.Electrical.AC.OnePhase.Storage.Battery bat_loss_acdc(
        etaCha=1,
        etaDis=1,
        SOC_start=0.5,
      ...
      Modelica.Blocks.Interfaces.RealInput Load annotation (Placement(
            transformation(extent={{-182,-90},{-160,-68}}),
                                                          iconTransformation(
              extent={{-182,-90},{-160,-68}})));
      Modelica.Blocks.Math.Gain gain(k=-1)
        annotation (Placement(transformation(extent={{-116,-86},{-102,-72}})));
      battery_controller battery_controller1
        annotation (Placement(transformation(extent={{254,-20},{288,8}})));
      mode_switch mode_switch1
        annotation (Placement(transformation(extent={{94,24},{124,50}})));
      Modelica.Blocks.Interfaces.RealOutput pvpower
        annotation (Placement(transformation(extent={{320,72},{340,92}}),
            iconTransformation(extent={{320,72},{340,92}})));
      Modelica.Blocks.Interfaces.RealOutput soc
        annotation (Placement(transformation(extent={{320,-36},{340,-16}}),
            iconTransformation(extent={{320,-78},{340,-58}})));
      Modelica.Blocks.Interfaces.IntegerOutput mode
        "Connector of Integer output signal" annotation (Placement(
            transformation(extent={{320,28},{340,48}}), iconTransformation(
              extent={{200,-8},{220,12}})));
    equation
      connect(grid.terminal,RL. terminal)
                                         annotation (Line(
          points={{-108,-22},{-108,-44},{-4,-44}},
          color={0,120,120},
          smooth=Smooth.None));
      connect(weaDat.weaBus,HDifTil. weaBus) annotation (Line(
          points={{-106,108},{-82,108},{-82,127},{-74,127}},
          color={255,204,51},
          thickness=0.5,
          smooth=Smooth.None));
      connect(weaDat.weaBus,HDirTil. weaBus) annotation (Line(
          points={{-106,108},{-82,108},{-82,93},{-74,93}},
          color={255,204,51},
          thickness=0.5,
          smooth=Smooth.None));
      connect(HDifTil.H,G. u1) annotation (Line(
          points={{-59.3,127},{-50,127},{-50,113.6},{-37.2,113.6}},
          color={0,0,127},
          smooth=Smooth.None));
      connect(HDirTil.H,G. u2) annotation (Line(
          points={{-59.3,93},{-50,93},{-50,106},{-37.2,106},{-37.2,106.4}},
          color={0,0,127},
          smooth=Smooth.None));
      connect(G.y,pvSimple. G) annotation (Line(
          points={{-23.4,110},{9,110},{9,66.2}},
          color={0,0,127},
          smooth=Smooth.None));
      connect(pvSimple.terminal,RL. terminal) annotation (Line(
          points={{-2,53},{-54,53},{-54,-44},{-4,-44}},
          color={0,120,120},
          smooth=Smooth.None));
      connect(bat_loss_acdc.terminal, RL.terminal) annotation (Line(points={{2,0},{
              -54,0},{-54,-44},{-4,-44}},          color={0,120,120}));
      connect(Load, gain.u)
        annotation (Line(points={{-171,-79},{-117.4,-79}},
                                                        color={0,0,127}));
      connect(gain.y, RL.Pow) annotation (Line(points={{-101.3,-79},{80,-79},{
              80,-44},{22,-44}},
                             color={0,0,127}));
      connect(gain.y, battery_controller1.load) annotation (Line(points={{-101.3,
              -79},{80,-79},{80,-8.2},{253,-8.2}},
                                                 color={0,0,127}));
      connect(gain.y, mode_switch1.load) annotation (Line(points={{-101.3,-79},
              {80,-79},{80,33.1},{92.9,33.1}},                      color={0,0,
              127}));
      connect(bat_loss_acdc.SOC, mode_switch1.soc) annotation (Line(points={{23,6},{
              30,6},{30,37.9},{92.9,37.9}},                              color={0,
              0,127}));
      connect(mode_switch1.y, battery_controller1.u) annotation (Line(points={{125,
              38.2},{144,38.2},{144,-2.8},{253,-2.8}},     color={255,127,0}));
      connect(pvSimple.P, battery_controller1.pvpower) annotation (Line(points={{21.1,
              60.7},{150,60.7},{150,-3},{253,-3}},      color={0,0,127}));
      connect(battery_controller1.y_battery, bat_loss_acdc.P) annotation (Line(
            points={{289,-6},{296,-6},{296,16},{12,16},{12,10}},    color={0,0,
              127}));
      connect(pvSimple.P, mode_switch1.pvpower) annotation (Line(points={{21.1,
              60.7},{80,60.7},{80,43.1},{92.9,43.1}}, color={0,0,127}));
      connect(bat_loss_acdc.SOC, soc) annotation (Line(points={{23,6},{30,6},{
              30,-26},{330,-26}}, color={0,0,127}));
      connect(pvSimple.P, pvpower) annotation (Line(points={{21.1,60.7},{312,
              60.7},{312,82},{330,82}}, color={0,0,127}));
      connect(mode_switch1.y, mode) annotation (Line(points={{125,38.2},{192,
              38.2},{192,38},{330,38}}, color={255,127,0}));
      annotation (Diagram(coordinateSystem(extent={{-160,-120},{320,140}})), Icon(
            coordinateSystem(extent={{-160,-120},{320,140}})),
        experiment(StopTime=31536000, __Dymola_Algorithm="Dassl"));
    end model_stategraph;
    ...
    model mode_switch
      ...
      Buildings.Controls.OBC.CDL.Conversions.BooleanToInteger booToIntDor(
          final integerTrue=Integer(TEES.PV_battery_stategraph.battery_mode.Dormant),
      ...
      Buildings.Controls.OBC.CDL.Conversions.BooleanToInteger booToIntCha(
          final integerTrue=Integer(TEES.PV_battery_stategraph.battery_mode.Charging),
          final integerFalse=0)
      ...
      Buildings.Controls.OBC.CDL.Conversions.BooleanToInteger booToIntDis(
          final integerFalse=0, final integerTrue=Integer(TEES.PV_battery_stategraph.battery_mode.Discharging))
    ...
    model mode_switch_test
      mode_switch mode_switch1
        annotation (Placement(transformation(extent={{-14,-12},{16,14}})));
      Modelica.Blocks.Sources.TimeTable pvpower(table=[0,0; 30,80; 60,80; 90,80;
            120,80; 150,80; 180,80])
        annotation (Placement(transformation(extent={{-82,24},{-62,44}})));
      Modelica.Blocks.Sources.TimeTable soc(table=[0.0,0.2; 30,0.2; 60,0.2; 90,
            0.2; 120,0.2; 150,0.2; 180,0.2])
        annotation (Placement(transformation(extent={{-82,-8},{-62,12}})));
      Modelica.Blocks.Sources.TimeTable load(table=[0,0; 30,-100; 60,-100; 90,-100;
            120,-100; 150,-100; 180,-100])
        annotation (Placement(transformation(extent={{-82,-38},{-62,-18}})));
      Modelica.Blocks.Interfaces.IntegerOutput y
        "Connector of Integer output signal"                                          annotation (Placement(
            transformation(extent={{100,-8},{120,12}}),  iconTransformation(extent={{200,-8},
                {220,12}})));
    equation
      connect(load.y, mode_switch1.load) annotation (Line(points={{-61,-28},{
              -20,-28},{-20,-2.9},{-15.1,-2.9}}, color={0,0,127}));
      connect(soc.y, mode_switch1.soc)
        annotation (Line(points={{-61,2},{-15.1,1.9}}, color={0,0,127}));
      connect(pvpower.y, mode_switch1.pvpower) annotation (Line(points={{-61,34},
              {-20,34},{-20,7.1},{-15.1,7.1}}, color={0,0,127}));
      connect(mode_switch1.y, y) annotation (Line(points={{17,2.2},{96,2.2},{96,
              2},{110,2}}, color={255,127,0}));
      annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
            coordinateSystem(preserveAspectRatio=false)));
    end mode_switch_test;
    ...
    model battery_controller "change model for the battery"
      ...
      Modelica.Blocks.Sources.IntegerExpression dorMod(y=Integer(TEES.PV_battery_stategraph.battery_mode.Dormant))
        "Dormant mode"
    ...
    equation
      connect(dorMod.y, isDor.u2) annotation (Line(points={{-35,24},{20,24}},
                       color={255,127,0}));
      connect(u, isDor.u1)
        annotation (Line(points={{-110,32},{20,32}}, color={255,127,0}));
      ...
      connect(load, add.u1) annotation (Line(points={{-111,-23},{12,-23},{12,
              -30},{20,-30}}, color={0,0,127}));
      annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,
                -140},{240,140}})),             Diagram(coordinateSystem(
              preserveAspectRatio=false, extent={{-100,-140},{240,140}})));
    end battery_controller;

    model state_graph_test
      PV_battery_stategraph.model_stategraph pV_Battery_stategraph
        annotation (Placement(transformation(extent={{-30,8},{18,36}})));
      Modelica.Blocks.Sources.Constant input_for_load1(k=600)
        annotation (Placement(transformation(extent={{-82,42},{-62,62}})));
    equation
      connect(input_for_load1.y, pV_Battery_stategraph.Load) annotation (Line(
            points={{-61,52},{-36,52},{-36,12.4154},{-31.1,12.4154}}, color={0,
              0,127}));
      annotation (
